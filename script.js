// LISTA ORIGINAL (itemsBase) - respetado tal cual con sus propiedades
let itemsBase = [
  ...
  {
    name: "GALLETA SELLADA CON BOLSA - D:12Hrs V:2 DIAS",
    descongelamiento: 12,
    vida: 48
  },
  {
    name: "3 LECHES VAINILLA - D:12H V:3 DIAS ",
    descongelamiento: 12,
    vida: 72
  },
  {
    name: "3 LECHES CHOCOLATE - D:12H V:3 DIAS ",
    descongelamiento: 12,
    vida: 72
  },
  ...
  {
    name: " TOCINO BOLSA SELLADA - D:2 DIAS V:6 DIAS",
    descongelamiento: 48,
    vida: 144
  },
  {
    name: " POP CORN CONGELADO MDO -D:4 DIAS V:2 DIAS",
    descongelamiento: 96,
    vida: 48
  },
  {
    name: " FILETE KRUNCHY CONGELADO MDO-D:4 DIAS V:2 DIAS ",
    descongelamiento: 96,
    vida: 48
  },
  {
    name: " HOT WING CONGELADO MARINADO -D:4 DIAS V:2 DIAS ",
    descongelamiento: 96,
    vida: 48
  },
  {
    name: " POLLO PICANTE CONGELADO MARINADO -D:4 DIAS V:2 DIAS ",
    descongelamiento: 96,
    vida: 48
  },
  {
    name: " POLLO ETC CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " POLLO RO CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " CRISPY STRIP CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " HOT WING CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " FILETE ETC CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " FILETE KRUNCHY CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " TENDER CONGELADO CRUDO -D:4 DIAS V:3 DIAS ",
    descongelamiento: 96,
    vida: 72
  },
  {
    name: " TENDER CONGELADO MARINADO -D:4 DIAS V:2 DIAS ",
    descongelamiento: 96,
    vida: 48
  },
  {
    name: " POLLO RO MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial",
    Marinado: 2,
    vida: 46
  },
  {
    name: " POLLO ETC MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " TENDER MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " HOTWING MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " NUGGET MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " STRIP MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " FILETE ETC MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " FILETE KRUNCHY MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  {
    name: " POP CORN MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  ...
  {
    name: " POLLO PICANTE MARINADO- M:2 Hrs V:2 DIAS desde fecha inicial ",
    Marinado: 2,
    vida: 46
  },
  ...
  {
    name: " TORTILLA CONGELADA -D:13Hrs V:1 DIA desde fecha inicial ",
    descongelamiento: 13,
    vida: 11
  }
];
...
function calculateExpiry() {
  ...
  if (!selectedOption) {
    document.getElementById("result").textContent =
      "Por favor, selecciona un ítem.";
    return;
  }
  const item = JSON.parse(selectedOption.value);
  const fechaInicialValue = document.getElementById("dateInput").value;
  const fechaInicial = fechaInicialValue ? new Date(fechaInicialValue) : null;

  if (!fechaInicial || isNaN(fechaInicial.getTime())) {
    document.getElementById("result").textContent =
  ...
  if (item.descongelamiento) {
    fechaVencimiento.setHours(
      fechaVencimiento.getHours() + item.descongelamiento
    );
  ...
  document.getElementById(
    "result"
  ).textContent = `Vencimiento: ${fechaVencimiento.toLocaleString("es-ES", {
    hour12: false
  })}`;
...
function renderSticker(nombre, fechaP, fechaL, fechaV) {
  ...
  const headerEl = document.getElementById("stickerHeader");
  headerEl.textContent = dias[dia];
  headerEl.style.backgroundColor = colores[dia];

  document.getElementById("pColumn").innerHTML = `P<br>${formatDateOnly(
    fechaP
  )}<br>H:<br>${formatHourOnly(fechaP)}`;
  document.getElementById("lColumn").innerHTML = fechaL
    ? `L<br>${formatDateOnly(fechaL)}<br>H:<br>${formatHourOnly(fechaL)}`
    : `L<br><br>H:<br>`;
  document.getElementById("vColumn").innerHTML = `V<br>${formatDateOnly(
    fechaV
  )}<br>H:<br>${formatHourOnly(fechaV)}`;

  const nombreCorto = nombre.split("-")[0].trim();
  document.getElementById("stickerProducto").textContent = nombreCorto;

  // Asignar clases opcionales por día (para estilos)
  headerEl.className = "sticker-header"; // reset
  const dayClasses = [
    "domingo",
    "lunes",
    "martes",
  ...
  headerEl.classList.add(dayClasses[dia]);
...
function renderAdminTable() {
  ...
  items.forEach((item, index) => {
    table.innerHTML += `
      <tr>
        <td><input value="${item.name.replace(
          /"/g,
          "&quot;"
        )}" onchange="items[${index}].name=this.value"></td>
        <td><input type="number" value="${
          item.Marinado || ""
        }" onchange="items[${index}].Marinado=this.value?parseInt(this.value):undefined"></td>
        <td><input type="number" value="${
          item.descongelamiento || ""
        }" onchange="items[${index}].descongelamiento=this.value?parseInt(this.value):undefined"></td>
        <td><input type="number" value="${
          item.vida
        }" onchange="items[${index}].vida=parseInt(this.value)"></td>
...
// Inicializar (si se desea)
document.addEventListener("DOMContentLoaded", function () {
  // Si quieres precargar items en la pantalla principal cuando se muestre:
  loadItems();
});
